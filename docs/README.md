...
Evita el acoplamiento rígido y la necesidad de modificar el código existente cada vez que se añade un nuevo comportamiento (cumpliendo con el principio Abierto/Cerrado).

Al usar condicionales (`if/else` o `switch`), el sistema se vuelve difícil de mantener y propenso a errores, ya que cualquier cambio exige buscar y modificar cada estructura condicional en el proyecto. Con el polimorfismo, el comportamiento se delega a las subclases o implementaciones; si se requiere un nuevo comportamiento, simplemente se crea una nueva clase sin tocar el código que la consume.
...
* **Hilos seguros (Thread-safety):** En entornos concurrentes/multihilo, varios hilos pueden compartir el objeto sin riesgo de corrupción de datos o condiciones de carrera (race conditions), eliminando la necesidad de sincronización compleja.
* **Integridad de datos:** Evita efectos secundarios no deseados causados por referencias compartidas (donde un componente modifica un objeto sin que otros se enteren).
* **Consistencia:** Los objetos inmutables son ideales como claves en mapas (`HashMap`) o elementos en conjuntos (`HashSet`), asegurando que su `hashCode` nunca cambie.
...
* **Pérdida de control y validación:** Cualquier clase externa puede asignar valores inválidos o incoherentes a los atributos (por ejemplo, una edad negativa o un saldo menor a cero).
...
* **Falta de flexibilidad:** No se pueden crear atributos de "solo lectura" o realizar operaciones lógicas ocultas (como registrar un log o activar un evento) al momento de modificar un valor.
...
Para lograrlo, el diseño original debe depender de abstracciones (interfaces o clases abstractas). Cuando se requiere una nueva funcionalidad, se crea una nueva clase que implemente la interfaz o herede de la clase abstracta correspondiente. De este modo, el comportamiento del sistema se extiende agregando código nuevo, mientras que las clases preexistentes permanecen intactas ("cerradas" a la modificación), reduciendo drásticamente el riesgo de introducir regresiones (errores en código que ya funcionaba).
...
A continuación se presenta una clase `Invoice` (Factura) que vulnera el principio, ya que se encarga de gestionar los datos de la factura, calcular los impuestos, guardarlos en la base de datos y enviar un correo electrónico.
